...
# (1, 2) **COORDENADAS, REGIONES, SUBREGIONES, PAÍSES MENOS DESARROLLADOS Y SIN LITORAL**
...
try: # Los primeros dos df se construyen a la par porque se filtran entre sí
    respuesta_m49 = requests.get(url_m49, headers=headers, timeout=10)    
    respuesta_m49.raise_for_status()
    ...
    respuesta_coordenadas = requests.get(url_coordenadas, params=params, timeout=10)
    respuesta_coordenadas.raise_for_status()
    ...
    df_m49 = pd.read_csv('fuentes_de_datos/m49.csv', keep_default_na=False) # Para que Namibia (NA) no pase a ser NaN
    # df auxiliar para agregrar códigos en las tablas
    df_alfa3_codigos = df_m49[['cod_m49', 'iso3_m49']]
    print('Operación finalizada.')
    print('2. Procesando ubicaciones...')
    df_coordenadas = pd.read_csv('fuentes_de_datos/coordenadas.csv', keep_default_na=False)
    print('Operación finalizada.')
    # Creamos los conjuntos de códigos que utilizamos más adelante    
    codigos_iso3 = set(df_coordenadas.iso3_coord.unique()) # Conjunto de códigos ISO 3166 alfa-3    
    codigos_m49 = set(df_m49.cod_m49.unique()) # Conjunto inicial de códigos globales m49
...
headers = {'User-Agent': 'Mozilla/5.0'}
try:
    respuesta_poblas = requests.get(url_poblaciones, headers=headers, timeout=10)
    respuesta_poblas.raise_for_status()
    ...
    df_poblaciones = pd.read_csv('fuentes_de_datos/poblaciones.csv', keep_default_na=False)
    print('Operación finalizada.')
...
try:    
    respuesta_migras = requests.get(url_migraciones, headers=headers, timeout=10)
    respuesta_migras.raise_for_status()
    ...
    df_migraciones = pd.read_csv('fuentes_de_datos/migraciones.csv', keep_default_na=False)
    df_migras_original = pd.read_csv('fuentes_de_datos/migraciones_original.csv', keep_default_na=False)
    print('Operación finalizada.')
...
try:    
    ...
    headers = {'User-Agent': 'Mozilla/5.0'}
    respuesta = requests.get(url_hdi, headers=headers, timeout=10)
    respuesta.raise_for_status()
    ...
    df_economicas = pd.read_csv('fuentes_de_datos/economicas.csv', keep_default_na=False)
    print('Operación finalizada.')
